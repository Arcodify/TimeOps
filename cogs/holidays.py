...
class Holidays(commands.Cog):
    ...
    async def _get_holidays(self, guild_id, upcoming_only=False):
    ...
    async def check_holiday_overlap(self, guild_id, start_date, end_date):
        """Returns list of holidays that overlap with a date range."""
        holidays = await self._get_holidays(guild_id)
        overlapping = []
        for h in holidays:
    ...
    async def holiday_list(self, interaction: discord.Interaction, upcoming_only: bool = True):
        holidays = await self._get_holidays(str(interaction.guild_id), upcoming_only=upcoming_only)
    ...
    @app_commands.checks.has_permissions(manage_guild=True)
    async def holiday_delete(self, interaction: discord.Interaction, date: str):
        async with aiosqlite.connect(self.db.path) as db:
    ...
    async def holiday_upcoming(self, interaction: discord.Interaction, count: int = 5):
        holidays = await self._get_holidays(str(interaction.guild_id), upcoming_only=True)

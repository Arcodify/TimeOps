...
class Reminders(commands.Cog):
    ...
    async def reminder_loop(self):
        ...
        for config in configs:
            if config["reminder_time"] == now_str:
                await self._send_reminders(config["guild_id"], config.get("channel_id"))

    async def _send_reminders(self, guild_id: str, channel_id: str = None):
        """DM all users currently clocked in."""
        guild = self.bot.get_guild(int(guild_id))
        if not guild:
            return

        # Find all currently clocked-in users for this guild
        import aiosqlite as _aio
        async with _aio.connect(self.db.path) as db:
        ...
        # Also post a summary to the optional channel
    ...
    @app_commands.checks.has_permissions(manage_guild=True)
    async def reminder_test(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        config = await self._get_config(str(interaction.guild_id))
        await self._send_reminders(str(interaction.guild_id), config.get("channel_id"))
